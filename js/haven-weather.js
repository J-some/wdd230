const apiURL =
  "https://api.openweathermap.org/data/2.5/weather?id=5604473&units=imperial&APPID=401afecdb37336a7e7a10503e1bf429b";
fetch(apiURL)
...
const forecastapiURL =
  "https://api.openweathermap.org/data/2.5/forecast?id=5604473&units=imperial&APPID=401afecdb37336a7e7a10503e1bf429b";
fetch(forecastapiURL)
  .then((response) => response.json())
  .then((jsObject) => {
    console.log(jsObject);

    const weather = jsObject.list.filter((x) => x.dt_txt.includes("18:00:00"));
    console.log(weather);
...
fetch(requestURL)
  ...
  .then(function (jsonObject) {
    ...
    towns.forEach((town) => {
      if (["Fish Haven"].includes(town.name)) {
        ...
        events = document.createElement("p");
        events.innerHTML = `${town.events[3]}`;
        card.appendChild(events);
